...
# 全域常數
VERSION = "1.0"
APP_TITLE = f"進階檔案搜尋システム v{VERSION}"
...
def search_files(keyword, folder_path, status_var):
    ...
    def process_folder(path, is_shortcut_target=False, shortcut_source=""):
        ...
        try:
            ...
            status_var.set(f"アクセス拒否: {path}")
            root.update_idletasks()
        except Exception as e:
            status_var.set(f"エラー: {path} - {e}")
            root.update_idletasks()
    ...
    # 搜尋完成後更新狀態
    if stop_search_flag:
        status_var.set("検索中止しました")
    else:
        status_var.set(f"検索完了、{len(matches)}個のファイルが見つかりました")
...
def open_file(filepath):
    ...
    try:
        ...
        status_var.set(f"ファイルを開けません: {e}")

def stop_search():
    """停止搜尋"""
    global stop_search_flag
    stop_search_flag = True
    status_var.set("検索を中止しています...")
    # 啟用搜尋按鈕
...
def perform_search():
    ...
    if not keyword or not folder_path:
        ...
        result_text.insert(END, "キーワードとフォルダパスを入力してください！\n", "warning")
        return

    # 檢查資料夾路徑是否存在
    if not os.path.isdir(folder_path):
        result_text.delete("1.0", END)
        result_text.insert(END, f"エラー：フォルダパス '{folder_path}' が存在しません！\n", "warning")
        return
    ...
    result_text.delete("1.0", END)
    result_text.insert(END, "検索中、お待ちください...\n", "info")
    result_text.insert(END, f"検索フォルダ: {folder_path}\n", "path")
    result_text.insert(END, f"キーワード: {keyword}\n\n", "bold")
    root.update_idletasks()
    ...
    def update_results():
        ...
        if file_mapping:
            result_text.insert(END, f"検索完了、{len(file_mapping)}個のファイルが見つかりました\n", "count")
            result_text.insert(END, f"検索フォルダ: {folder_path}\n", "path")
            result_text.insert(END, f"キーワード: {keyword}\n\n", "bold")
            ...
            result_text.insert(END, "ファイルが見つかりませんでした！\n\n", "warning")
            result_text.insert(END, f"検索フォルダ: {folder_path}\n", "path")
            result_text.insert(END, f"キーワード: {keyword}\n", "bold")
...
def on_file_select(event):
    ...
    try:
        ...
        status_var.set(f"ファイルクリック時にエラーが発生しました: {e}")
        print(f"點擊檔案時發生錯誤: {e}")
...
def create_tooltip(widget, text):
    ...
    def enter(event):
        ...
        label = tk.Label(tooltip, text=text, background="#FFF7E6", relief="solid", borderwidth=1)
        label.pack()
...
def show_about_dialog():
    ...
    about_window.title(f"について {APP_TITLE}")
    about_window.geometry("400x300")
    about_window.resizable(False, False)
    about_window.configure(bg="#FFFAF0")
    ...
    # 功能說明
    feature_frame = tk.Frame(about_window, bg="#FFFAF0", padx=20)
    feature_frame.pack(fill="both", expand=True)

    features = [
        "✓ ファイルキーワード検索",
        "✓ ショートカットフォルダの再帰的な検索をサポート",
        "✓ 制限されたパスを自動的にスキップ",
        "✓ いつでも検索を停止できる",
    ...
    # 底部按鈕
    btn_frame = tk.Frame(about_window, bg="#FFFAF0", pady=10)
...
def clear_results():
    """清除搜尋結果"""
    result_text.delete("1.0", END)
    result_text.insert(END, f"進階檔案搜尋システム v{VERSION}へようこそ\n\n", "info")
    result_text.insert(END, "1. ファイルのキーワードを入力してください\n")
    result_text.insert(END, "2. 検索するフォルダを選択してください\n")
    result_text.insert(END, "3. 「検索開始」ボタンをクリックしてください\n\n")
    result_text.insert(END, "✨ このシステムはショートカットフォルダの再帰的な検索をサポートし、フォルダの最下層まで探索できます ✨\n", "highlight")

# 建立 GUI 主視窗
root = tk.Tk()
root.title(APP_TITLE)
root.geometry("800x600")
root.configure(bg="#FFF7E6")  # 淡米色背景

# 設定主題色彩 - 溫暖柔和的日系配色
primary_color = "#D7A98C"     # 淺褐色
secondary_color = "#FFF7E6"   # 米色
accent_color = "#B3A369"      # 黃褐色
text_color = "#5D4037"        # 深褐色
...
# 建立自定義的字型
title_font = font.Font(family="BIZ UDPゴシック", size=14, weight="bold")
label_font = font.Font(family="BIZ UDPゴシック", size=10)
button_font = font.Font(family="BIZ UDPゴシック", size=10, weight="bold")
result_font = font.Font(family="BIZ UDPゴシック", size=10)
bold_font = font.Font(family="BIZ UDPゴシック", size=10, weight="bold")
path_font = font.Font(family="BIZ UDPゴシック", size=9)
...
file_menu.add_command(label="フォルダを開く", command=browse_folder)
...
tools_menu.add_command(label="検索結果をクリア", command=clear_results)

# 幫助選單
help_menu = tk.Menu(menu_bar, tearoff=0)
menu_bar.add_cascade(label="ヘルプ", menu=help_menu)
help_menu.add_command(label="について", command=show_about_dialog)
...
# 配置樣式
style.configure("TEntry", padding=8, relief="flat", background="#FFFBF0")
style.configure("TButton",
                ...
                foreground="#FFFFFF")
style.map("TButton",
          background=[("active", primary_color), ("pressed", "#C69C80")],
...
style.map("Search.TButton",
          background=[("active", accent_color), ("pressed", "#A39159")],
          foreground=[("active", "#FFFFFF"), ("pressed", "#FFFFFF")])
...
style.map("Stop.TButton",
          background=[("active", "#CB8E79"), ("pressed", "#B87E69")],
          foreground=[("active", "#FFFFFF"), ("pressed", "#FFFFFF")])

# 標題區域
header_frame = tk.Frame(root, bg="#FFF7E6", pady=10)
...
# 創建主體框架
main_frame = tk.Frame(root, bg="#FFF7E6", padx=20, pady=10)
...
create_tooltip(keyword_entry, "検索するファイルのキーワードを入力してください")
...
create_tooltip(folder_path_entry, "検索するフォルダパスを選択してください")
...
# 按鈕區域
button_frame = tk.Frame(root, bg="#FFF7E6", pady=10)
button_frame.grid(row=2, column=0, columnspan=3, sticky="ew")

# 建立按鈕容器，並排
button_container = tk.Frame(button_frame, bg="#FFF7E6")
...
# 狀態顯示
status_frame = tk.Frame(root, bg="#FFF7E6")
...
result_frame = tk.Frame(root, bg="#FFF7E6", padx=20, pady=10)
...
result_text.tag_configure("path", font=path_font, foreground="#7D6E5D")
result_text.tag_configure("info", foreground="#8C715F")
result_text.tag_configure("warning", foreground="#B96455")
result_text.tag_configure("count", font=bold_font, foreground=primary_color)
result_text.tag_configure("highlight", background="#FBEBC6", font=bold_font)
...
# 底部版本和版權信息
footer_frame = tk.Frame(root, bg="#FFF7E6", pady=5)
footer_frame.grid(row=5, column=0, columnspan=3, sticky="ew")

# 建立兩個標籤，一個用於版本信息，一個用於GitHub連結
version_label = tk.Label(
    footer_frame,
    text=f"バージョン {VERSION} | 開発者：陳兪源",
    font=("BIZ UDPゴシック", 8),
    bg="#FFF7E6",
    fg="#8D7866",
...
github_label = tk.Label(
    footer_frame,
    text="GitHub: https://reurl.cc/Nbzm2p",
    font=("BIZ UDPゴシック", 8),
    bg="#FFF7E6",
    fg="#0066CC",  # 藍色，表示可點擊的連結
    cursor="hand2"
)
...
def open_github_link(event):
    import webbrowser
    webbrowser.open("https://reurl.cc/Nbzm2p")
...
result_text.insert(END, "3. 「検索開始」ボタンをクリックしてください\n\n")
result_text.insert(END, "✨ このシステムはショートカットフォルダの再帰的な検索をサポートし、フォルダの最下層まで探索できます ✨\n", "highlight")
result_text.insert(END, "📋 バージョン情報：\n", "bold")
result_text.insert(END, "v1.0 - 初期バージョン：ファイルキーワード検索、ショートカットフォルダのサポート\n", "path")
